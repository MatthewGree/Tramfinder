...
case class Graph private[graph](private val stops: Map[Int, TramStop], private val edges: Map[Int, Seq[Edge]]) {
  def getStop(stopId: Int): Option[TramStop] = stops.get(stopId)
  def getAllStops: Iterable[TramStop] = stops.values
  def getStopEdges(stop: TramStop): Seq[Edge] = edges.getOrElse(stop.id, Seq.empty)
}
